...
"""
DSpark V4.1 drafter runtime modules (DeepSeek-V4.1 MTP component). Extends DSparkAttention
with V4.1-specific rope semantics: no per-head query RMS norm, kv norm applied separately.
...
position, stored paged so draft and target cache layouts stay aligned.
"""
...
class DSparkV41Attention(DSV4Attention):
    """
    Compressor-less DSV4 attention with DSpark draft semantics and V4.1 rope: same projection
    set and tensor keys as a trunk sliding layer, but a paged main-kv cache layer and
    block-parallel non-causal forward. V4.1: no per-head q norm, kv norm applied separately.
    """

    def __init__(self, *args, q_head_norm: bool = False, **kwargs):
        # V4.1 drafter doesn't use per-head q norm
        super().__init__(*args, q_head_norm = q_head_norm, **kwargs)
    ...
    def forward(self, x: torch.Tensor, params: dict, out_dtype: torch.dtype | None = None):
        ...
        sinks. V4.1: apply kv norm separately when q_head_norm is False."""
        bsz, s, _ = x.shape
        device = x.device
        cache = params["cache"]
        ...
        # Projections: q_a / q_norm / q_b for q, wkv for kv
        ...
        # V4.1 rope: apply kv norm separately (q_head_norm = False)
        if not self.q_head_norm:
            kv = kv.clone()  # Don't modify in-place during in_place=False op
    ...
    def update_kv_rows(self, main_x: torch.Tensor, params: dict):
        ...
        projected tap state main_x (bsz, s, hidden) fp16. V4.1: apply kv norm separately."""
        cache = params["cache"]
        kl = cache.layers[(self.layer_idx, params.get("layer_instance", 0))]
        block_table = get_for_device(params, "block_table", self.device)
        ...
        # V4.1 rope: apply kv norm separately
        if not self.q_head_norm:
            kv = kv.clone()
            kv.copy_(self.kv_norm.forward(kv, {}, out_dtype = torch.half).view_as(kv))
            k_w = None
        ...
        ext.rope(
            kv, kv, None, None,
            self._rope_type(), 0, positions, None,
            int(RopeStyle.GPTJ), 1.0, k_w, None,
            self.rms_norm_eps, 0.0, 0.0, 0, 1, D - rd,
        )
        if dspark_fp8_kv:
            fp8_fake_quant_(kv[..., : D - rd])
